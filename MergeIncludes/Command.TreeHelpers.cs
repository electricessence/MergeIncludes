...
public sealed partial class CombineCommand
{
	...
	/// </summary>
	private static void AddChildrenToFolderGroupedTree(
		TreeNode parentNode,
		string parentPath,
		...
		try
		{
				...
				!string.Equals(path, parentPath, StringComparison.OrdinalIgnoreCase)))
			{
				try
				{
					...
					if (!filesByDirectory.TryGetValue(dirPath, out List<FileInfo>? value))
					{
						value = [];
						filesByDirectory[dirPath] = value;
					}
				...
				else
				{
					try
					{
						...
						else
						{
							...
							// Use folder icon for better console compatibility
							var folderText = new Text($"📁 {folderName}", new Style(Color.Yellow3));
	...
	/// </summary>
	private static void AddChildrenToRepeatsOnlyTree(
		TreeNode parentNode,
		string parentPath,
		Dictionary<string, List<string>> fileRelationships,
		Dictionary<string, int> fileIds,
	...
	/// Adds children to a simple tree node format (just names and IDs)
	/// </summary>
	private static void AddChildrenToSimpleTree(
		TreeNode parentNode,
		string parentPath,
		Dictionary<string, List<string>> fileRelationships,
		Dictionary<string, int> fileIds,
	...
	/// </summary>
	private static void AddChildrenToFolderTreeWithText(
		TreeNode parentNode,
		string parentPath,
		Dictionary<string, List<string>> fileRelationships,
		DirectoryInfo workspaceRoot,
	...
	/// </summary>
	private static void AddChildrenToRelativePathTreeWithText(
		TreeNode parentNode,
		string parentPath,
		Dictionary<string, List<string>> fileRelationships,
		...
		HashSet<string>? visitedPaths = null)
	{
		...
		foreach (var childPath in children)
		{
			...
			{
				var parts = relativePath.Split([" (in "], 2, StringSplitOptions.None);
	...
	/// </summary>
	private static int CountAllFilesInTree(FileInfo rootFile, Dictionary<string, List<string>> fileRelationships)
	{
	...
	/// </summary>
	private static bool IsFileRepeatedInRelationships(string filePath, Dictionary<string, List<string>> fileRelationships)
	{
	...
	/// </summary>
	private static string GetIdFormat(int totalFiles)
	{
		if (totalFiles < 10)
			return "[{0}]";
		...
		HashSet<string> visitedPaths)
	{
		...
		if (!fileRelationships.TryGetValue(parentPath, out List<string>? value))
			return; foreach (var childPath in value)
		{
			try
			{
