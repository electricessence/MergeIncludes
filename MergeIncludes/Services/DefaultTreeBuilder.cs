...
public class DefaultTreeBuilder
{
    ...
    private List<string> BuildAlignedFolderLines(FileInfo rootFile, Dictionary<string, List<string>> fileRelationships, List<string> referenceLines)
    {
        ...
        for (int i = 0; i < referenceLines.Count; i++)
        {
            ...
            if (file?.Directory != null)
            {
                ...
                var isRootFile = fileName.Equals(rootFile.Name, StringComparison.OrdinalIgnoreCase);

                if (isRootFile)
                {
                    // Root file - show folder icon with continuation if there are child items
    ...
    private string GetFolderTreeLine(FileInfo file, Dictionary<string, FolderNode> folderHierarchy, bool isRootFile)
    {
        ...
        if (dirPath != null && folderHierarchy.TryGetValue(dirPath, out var node))
        {
            ...
            // Build tree structure for this path
            var result = "";
