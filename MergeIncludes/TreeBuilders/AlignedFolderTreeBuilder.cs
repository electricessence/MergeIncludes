...
public static class AlignedFolderTreeBuilder
{
    ...
    {        var nodeList = referenceNodes.ToList();
        if (!nodeList.Any()) return;

        string? lastDirectoryPath = baseDirectory.FullName;
        var folderNodes = new Dictionary<string, IHasTreeNodes>(); // Track folder nodes by path
        folderNodes[baseDirectory.FullName] = tree; // Root folder
 ...
 /// <summary>
    ...
    /// </summary>
    private static bool CanCreateNaturalHierarchy(List<ReferenceNode> nodeList, int currentIndex, ReferenceNode currentNode)
    {
